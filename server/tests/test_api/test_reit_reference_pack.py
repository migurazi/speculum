...
def test_builtin_pack_hash_unchanged() -> None:
    """빌트인 v1.0.0 pack content_hash 가 리츠 factor 도입 후에도 불변 (재현성 봉인).

    리츠 factor 는 별도 reference pack 이고 빌트인 v1.0.0 JSON 무변경이므로 기존 저장
    run 의 snapshot_versions frozen hash 와 계속 일치 (ADR-0023 D8). M7 #4 에서
    DEFAULT_PACK 이 v1.1.0 으로 bump 됐으므로 v1.0.0 을 명시 로드하여 byte-불변을
    검증한다 (frozen run 은 v1.0.0 을 재로드).
    """
    from app.services.factor_pack import load_builtin_pack

    v100 = load_builtin_pack("1.0.0")
    assert v100.pack_slug == "speculum-builtin"
    assert v100.computed_hash == _BUILTIN_HASH
