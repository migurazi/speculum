...
@pytest.fixture(scope="session")
def ecos_api_key() -> str | None:
    """한국은행 ECOS API key — 환경변수에서 fetch. 없으면 None.

    dart_api_key 와 동일 패턴 — secret 미주입 시 ECOS smoke 가 skip.
    """
...
@pytest.fixture(scope="session")
def kosis_api_key() -> str | None:
    """통계청 KOSIS API key — 환경변수에서 fetch. 없으면 None.

    dart_api_key 와 동일 패턴 — secret 미주입 시 KOSIS smoke 가 skip.
    """
