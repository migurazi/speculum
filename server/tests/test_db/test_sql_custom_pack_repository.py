...
# =============================================================================
# ADR-0032 D3 — import provenance(source_url) row metadata
# =============================================================================

@pytest.mark.parametrize("factory", _REPO_FACTORIES)
def test_save_roundtrips_source_url(db_session: Session, factory) -> None:
    """source_url(provenance)이 save → get/list 라운드트립으로 보존."""
    repo: CustomPackRepository = factory(db_session)
    url = "https://example.com/packs/p-1.0.0.json"
    saved = repo.save(user_id=_USER_A, body=_sealed_body(), source_url=url)
    assert saved.source_url == url

    fetched = repo.get(saved.id, user_id=_USER_A)
    assert fetched is not None
    assert fetched.source_url == url
    # 목록(list_for_user)도 동일 row 메타 노출.
    assert repo.list_for_user(user_id=_USER_A)[0].source_url == url


@pytest.mark.parametrize("factory", _REPO_FACTORIES)
def test_save_default_source_url_none(db_session: Session, factory) -> None:
    """source_url 미전달(editor 작성) → None(출처 없음)."""
    repo: CustomPackRepository = factory(db_session)
    saved = repo.save(user_id=_USER_A, body=_sealed_body())
...
@pytest.mark.parametrize("factory", _REPO_FACTORIES)
def test_source_url_does_not_affect_content_hash(
    db_session: Session, factory,
) -> None:
    """ADR-0032 D3 핵심 불변식 — source_url 은 content_hash 에 영향 0.

    provenance 가 있는 저장과 없는 저장의 content_hash 가 동일해야 한다(봉인은
    body 기준, source_url 은 row 메타). 같은 정의는 idempotent 라 별도 user 로
    각각 저장해 hash 만 비교한다.
    """
    repo: CustomPackRepository = factory(db_session)
    body = _sealed_body()
    with_url = repo.save(
        user_id=_USER_A, body=body,
        source_url="https://example.com/p.json",
    )
    without_url = repo.save(user_id=_USER_B, body=body)
    # 봉인 hash 는 body 만의 함수 — provenance 유무와 무관(D3 불변식).
    assert with_url.content_hash == without_url.content_hash
    # body 안에 source_url 이 새어들어가지 않음(봉인 파손 방지).
    assert "source_url" not in with_url.body


@pytest.mark.parametrize("factory", _REPO_FACTORIES)
def test_idempotent_resave_keeps_original_source_url(
    db_session: Session, factory,
) -> None:
    """같은 정의 재저장(idempotent) → 최초 source_url 유지(덮어쓰지 않음)."""
    repo: CustomPackRepository = factory(db_session)
    body = _sealed_body()
    first = repo.save(
        user_id=_USER_A, body=body,
        source_url="https://first.example.com/p.json",
    )
    # 같은 정의를 다른 source_url 로 재저장 — idempotent 라 최초 row 반환.
    second = repo.save(
        user_id=_USER_A, body=body,
        source_url="https://second.example.com/p.json",
    )
    assert first.id == second.id
    assert second.source_url == "https://first.example.com/p.json"
...
def test_orm_create_all_includes_custom_packs() -> None:
    ...
    try:
        ...
        assert {
            ...
            "visibility",
            # ADR-0032 D3 — import provenance(출처 URL) row 메타 컬럼.
            "source_url",
        } <= cols
