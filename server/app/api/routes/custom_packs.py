...
from app.api.dependencies.repositories import CustomPackRepoDep, PublisherRepoDep
...
async def save_custom_pack(
    ...
    publisher_repo: PublisherRepoDep,
    ...
    **v2 namespace 사칭 차단(M6 #4, ADR-0034 D4/D6)**: 봉인된 body 의 pack_slug 가
    v2(`@{publisher}/{slug}`)면, 발급 user(CurrentUserDep — IDOR 로 body 미수신)의
    claim handle 과 slug 의 publisher 일치를 강제한다(`verify_publisher_owns_slug`).
    미claim 또는 타 publisher 사칭 → 403. v1 slug(`user/`/`community/`)는 무관(통과).
    빌트인 canonical 사칭은 `_validate_and_seal` 의 derive_tier 게이트가 별도 차단
    (v2 는 community tier 라 통과하고, publisher 사칭은 본 verify 가 담당).
    """
    sealed = _validate_and_seal(body.pack)
    # v2 발급권 사칭 차단 — 봉인 body 의 권위 pack_slug 로 검사(클라이언트 위조 무관).
    try:
        verify_publisher_owns_slug(
            user_id=user.user_id,
            pack_slug=sealed["pack_slug"],
            publisher_repo=publisher_repo,
        )
    except PublisherImpersonationError as exc:
        # 미claim/타 publisher 사칭 v2 발급 → 403(Forbidden, ADR-0034 D4).
        raise HTTPException(status_code=403, detail=str(exc)) from exc
    try:
        # source_url(ADR-0032 D3 provenance)은 sealed body 와 분리 전달 — row 메타라
        # content_hash 봉인(_validate_and_seal)에 영향 0(provenance≠재현 입력).
        saved = repo.save(
            user_id=user.user_id, body=sealed, source_url=body.source_url,
        )
