...
async def save_run(
    ...
    # 2. data_versions freeze — M1 T48b. 정책 11 키 + (SQL session 있으면)
    #    as_of 시점 최신 성공 batch 의 krx/dart batch_id 2 키 명시 merge.
    #    Fake-only mode (session=None) 는 정책-only 11 키 (하위호환).
...
async def get_run_diff(
    ...
    M1 T48b — 비교 기준 `current` 는 snapshot 과 동일한 산출 경로
    (`collect_run_data_versions(snapshot.as_of, session)`) 를 사용 → 정책 11 키
    + batch_id 2 키. 같은 as_of 에서 새 배치가 성공하면 batch_id 변경이 diff 에
    노출되어 freshness banner 가 통지 (새 run = 다른 연산, 재현 hash 와는 무관).
    Fake-only mode (session=None) 면 정책-only 11 키 — 기존 11 키 run 과 일치하여
    빈 diff (하위호환).
...
async def reproduce_run_endpoint(
    ...
    # 2. _SnapshotIn → ScreenRunSnapshot 도메인 재구성.
    #    ScreenRunBuilder.build 가 canonical 정규화 + result_hash 재계산.
    #    재현은 user_id 무관 — user_id 는 snap_in 에서 pass-through 하되
    #    reproduce_run 은 그 값을 사용하지 않음 (ADR-0021 D5).
    #    None 필드는 이미 resolve_snapshot 의 필수 검증을 통과했으므로 안전.
    from uuid import uuid4 as _uuid4
    snapshot = ScreenRunBuilder.build(
        run_id=snap_in.id or _uuid4(),
