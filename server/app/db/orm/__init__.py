...
Publisher 인증 발급 (M6 #3, ADR-0034 D4, migration 0021):
    - publishers — 인증 user 의 publisher handle claim. user_id FK → users.id
      (격리 anchor) + handle + created_at. **append-only**(updated_at 없음 —
      claim 은 1 회성 사실). UNIQUE(user_id)로 user 1:1, handle unique Index 로
      인스턴스 유일(사칭 차단 — `@{handle}/...` v2 pack 발급권 단일 user 귀속).
      시스템 생성 0 — 인증 route + repository 만 생성(migration insert 없음).
...
stock_snapshots (factor 결과 precomputed) — 영속 layer(ORM·repo·migration) 추가 완료
(ⓓ slice 1). write 배치·screen read 경로 배선(§2.10 reproduce bypass)은 후속 cycle.
"""
