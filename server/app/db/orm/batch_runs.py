...
핵심 결정 (M1 지시서 T48a — Momus H1 "SoT 명시"):
    ...
    - **`source`** — "KRX" | "DART". `collect_batch_versions` (T48b) 가 source
      별 max(started_at) 성공 batch 를 조회하는 인덱스 key.
...
class BatchRunORM(Base):
    ...
    # "KRX" | "DART" — collect_batch_versions 의 source 별 조회 key.
