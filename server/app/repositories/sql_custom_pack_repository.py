...
class SqlCustomPackRepository(CustomPackRepository):
    ...
    def save(
        self, *, user_id: UUID, body: dict, source_url: str | None = None,
    ) -> CustomPack:
        ...
        if existing is not None:
            if existing.content_hash == content_hash:
                # 같은 정의 재저장 — idempotent. 기존 row 의 source_url 유지(최초
                # provenance 보존 — 봉인된 정의에 한 번만 귀속).
        ...
        record = CustomPack(
            ...
            created_at=_now(),
            # ADR-0032 D3 — import 출처. row 메타라 content_hash 무관.
