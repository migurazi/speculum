...
def extract_disclosure_facts(
    ...
    *,
    ...
    #
    # include_sentiment=True — LLM 출력은 absolute("매수/추천")뿐 아니라 sentiment
    # 가치판단("호재/악재/긍정적/부정적")까지 차단(ADR-0031 D2). 이 어휘는 LLM 생성
    # 텍스트 게이트에만 적용 — conformance CLI/미들웨어 기본 검사는 absolute 만이라
    # 본 모듈의 정책 설명 주석/docstring 이 자기 자신을 잡지 않는다(scope 분리).
    for field_path, value in facts.iter_text_fields():
        try:
            assert_clean(
                value,
                scope=CheckScope.SYSTEM,
                context=field_path,
