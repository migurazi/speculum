...
def derive_tier(pack_slug: str) -> TierName:
    """pack_slug → identity tier (ADR-0022 D8.1 / ADR-0002 D2 / ADR-0034 D3).

    canonical(빌트인) 은 불가침 tier(D8.2). community/custom 은 사용자 정의 tier.
    v2 namespace(`@{publisher}/{slug}`)는 community tier 재사용(ADR-0034 D3).
    """
    if pack_slug == _BUILTIN_SLUG:
        return "canonical"
    if pack_slug.startswith("community/"):
        return "community"
    # ADR-0034 D3 — v2 identity v2 namespace(`@{publisher}/{slug}`)는 **community
    # tier 재사용**(신 tier 신설 안 함 — TierName Literal·_tier_rank 변경 0). `@`
    # prefix 는 v1 pattern(speculum-builtin/`community/`/`user/`)과 **disjoint** 라
    # 기존 분기 무영향(v1 derive_tier 동작 byte-불변). 인증된 publisher 의 공유
    # pack 이므로 community(공유 pack) 의미와 일관.
    if pack_slug.startswith("@"):
        return "community"
    if pack_slug.startswith("user/"):
        return "custom"
    # schema pattern 이 위 넷 중 하나를 강제하나, 방어적으로 custom 취급
