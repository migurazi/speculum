...
# ADR-0034 D6 — v2 namespace pack(`@{publisher}/{slug}`)의 schema. v1 과 형태 동일
# 하나 pack_slug pattern 만 `@{publisher}/{slug}` 로 다르다. validate_schema 가
# body 의 `$schema` URI 로 v1/v2 validator 를 dispatch(아래 _VALIDATORS).
_SCHEMA_PATH_V2: Final[Path] = _REPO_ROOT / "shared" / "schemas" / "factor-pack-v2.json"
...
def _load_schema_file(path: Path) -> dict[str, Any]:
    """JSON schema 파일 로드 — module import 시 1회."""
    with path.open(encoding="utf-8") as f:
        return json.load(f)


def _load_schema() -> dict[str, Any]:
    """v1 JSON schema 로드 — 하위호환 alias(`_SCHEMA` 노출 유지)."""
    return _load_schema_file(_SCHEMA_PATH)


_SCHEMA: Final[dict[str, Any]] = _load_schema()
_SCHEMA_V2: Final[dict[str, Any]] = _load_schema_file(_SCHEMA_PATH_V2)

# pack body 의 `$schema` 필드 URI 값 — pack 정의가 자기 schema 의 `$id` 를 가리킨다
# (JSON Schema meta-schema 가 아님). validate_schema 가 이 URI 로 validator 를 고른다.
_SCHEMA_URI_V1: Final[str] = "https://speculum.dev/schemas/factor-pack-v1.json"
_SCHEMA_URI_V2: Final[str] = "https://speculum.dev/schemas/factor-pack-v2.json"

# ADR-0034 D6 — `$schema` URI → validator dispatch map. v1 단일 싱글톤(_VALIDATOR)을
# v1/v2 map 으로 확장: v2 namespace pack(`@{publisher}/{slug}`)이 v1 pattern 불일치로
# 거부되던 문제를 해소한다. **v1 byte-불변 보장**(§2.10): v1 URI/미지/부재 body 는
# 아래 validate_schema 가 v1 validator(=_VALIDATOR)를 그대로 쓴다.
_VALIDATOR: Final[Draft202012Validator] = Draft202012Validator(_SCHEMA)
_VALIDATOR_V2: Final[Draft202012Validator] = Draft202012Validator(_SCHEMA_V2)
_VALIDATORS: Final[dict[str, Draft202012Validator]] = {
    _SCHEMA_URI_V1: _VALIDATOR,
    _SCHEMA_URI_V2: _VALIDATOR_V2,
}
...
# AST 중첩 상한 — 정적 walk(`_ast_input_fields`/`_ast_ops` 등의 `_max_depth=64`)와
# 동일. 정상 pack 의 AST 깊이는 evaluator `max_ast_depth=32` 이하라 64 는 충분한
# 여유. jsonschema 재귀가 소비하는 frame(레벨당 수 개)을 감안해도 64×수개 < Python
# 기본 recursion limit(~1000) 안쪽이라 선검사 통과분은 안전하게 재귀 검증된다.
_MAX_AST_NESTING: Final[int] = 64


def _assert_ast_depth_within_limit(body: Any) -> None:
    """schema(jsonschema 재귀) 검증 **전에** factor AST 중첩 깊이를 선검사.

    `shared/schemas/factor-pack-v1.json` 의 `expression` 은 재귀 `oneOf` 라 깊은
    중첩 AST 가 jsonschema 의 재귀 검증에서 Python RecursionError 를 유발 → 미처리
    500 이 될 수 있다. `/factor-packs/validate`·`/import-check` 는 **인증 불요**라
    익명 DoS 표면이 된다(M4_PLAN §3 #1 가 명시 요구한 방어).

    본 선검사는 **명시 스택**(반복)으로 깊이만 재므로 자체 재귀 고갈이 없고, 한도
    초과 시 `SchemaValidationError` 로 fail-loud → validate_custom_pack 이 schema
    stage 이슈(valid=False)로 수렴(깔끔한 거부, 500 아님).

    body 가 기대 구조(factors/formula/ast)가 아니면 조용히 통과 — 구조 오류는 뒤
    이어지는 jsonschema 가 정식 보고(본 함수는 depth 만 책임).
    """
    ...
    for factor in factors:
        ...
        if not isinstance(formula, dict):
            continue
        # 명시 스택 — (node, depth). 재귀 없이 최대 깊이 추적.
        stack: list[tuple[Any, int]] = [(formula.get("ast"), 0)]
        while stack:
            node, depth = stack.pop()
            if not isinstance(node, dict):
                continue
            if depth > _MAX_AST_NESTING:
                raise SchemaValidationError(
                    f"factor '{factor.get('canonical_id', '<unknown>')}' 의 AST "
                    f"중첩 깊이가 {_MAX_AST_NESTING} 초과 — corruption 또는 DoS "
                    f"의심으로 거부."
                )
...
def _select_validator(body: dict[str, Any]) -> Draft202012Validator:
    """body 의 `$schema` URI 로 v1/v2 validator 를 선택 — ADR-0034 D6 dispatch.

    pack body 의 `$schema` 필드는 자기 정의 schema 의 `$id`(예:
    `https://speculum.dev/schemas/factor-pack-v2.json`)를 가리킨다. 그 URI 로
    `_VALIDATORS` map 에서 validator 를 고른다.

    **v1 byte-불변(§2.10 최우선)**: `$schema` 가 v1 URI 이거나, 미지의 값이거나,
    아예 없으면(부재) **v1 validator(default)** 를 쓴다. 이로써 기존 v1 body 의 검증
    동작이 100% 동일하게 보존된다(하위호환 — 기존 test_factor_pack 등 green). v2
    namespace pack 만 `$schema`=v2 URI 로 v2 validator(`@{publisher}/{slug}` slug
    pattern 허용)를 선택한다.
    """
    schema_uri = body.get("$schema") if isinstance(body, dict) else None
    # 미지/부재 → v1 default(.get default 가 v1 validator).
    return _VALIDATORS.get(schema_uri, _VALIDATOR)


def validate_schema(body: dict[str, Any]) -> None:
    """JSON schema 검증. 실패 시 SchemaValidationError + 첫 위반 경로.

    jsonschema 재귀 검증 전에 AST 깊이 선검사(`_assert_ast_depth_within_limit`)로
    깊은 중첩 입력의 RecursionError(→ 인증 불요 endpoint DoS)를 차단한다.

    ADR-0034 D6 — body 의 `$schema` URI 로 v1/v2 validator 를 dispatch(`_select_
    validator`). v1/미지/부재는 v1 validator(byte-불변), v2 URI 는 v2 validator
    (`@{publisher}/{slug}` slug 허용). 깊이 선검사·SchemaValidationError 경로는
    v1/v2 공통(M5 #1 DoS 가드 유지).
    """
    # 깊이 선검사 — jsonschema 재귀 진입 전(인증 불요 endpoint DoS 방어). v1/v2 공통.
    _assert_ast_depth_within_limit(body)
    validator = _select_validator(body)
    try:
...
def validate_forbidden_vocab(body: dict[str, Any]) -> None:
    ...
    **scope 주의**: 본 함수는 save·import 양쪽(`validate_custom_pack` 경유)에서 불린다.
    pack-level 공개 메타(`description`/`citation.title`)는 여기서 검사하지 **않는다** —
    private pack 은 그 메타를 USER_PRIVATE 로 보유 가능하고(ADR-0028 D2), 게이트는
    공유 진입(public-toggle) 또는 외부 import 시점이다. 외부 import 의 pack-메타
    게이트는 route(`_gate_import_pack_meta`, ADR-0032 D4)가 담당.
    """
    for factor in body["factors"]:
...
def validate_shared_meta(body: dict[str, Any]) -> None:
    """pack 공개 메타(citation.title + description)의 금지 어휘 — ADR-0032 D4.

    `validate_forbidden_vocab`(factor 정의, save+import 공유)과 **분리**된다. pack-level
    공개 메타는 **공유/import 시점에만** 검사한다 — private pack 은 자기 메타를
    USER_PRIVATE 로 보유 가능(ADR-0028 D2)하므로 save 경로(`validate_custom_pack`)는
    부적용. 본 함수는 **외부 import**(route `_gate_import_pack_meta`)와 **publish 전
    검증**(`tools/validate_pack.py`)이 호출한다 — 외부 저자의 advice 어휘가 다른
    사용자에게 렌더되기 전 차단(§2.2). public-toggle(ADR-0028 D2)과 동일 필드·USER_SHARED.

    Raises:
        ForbiddenVocabInPack: citation.title 또는 description 에 금지 어휘.
    """
    citation = body.get("citation")
    title = citation.get("title") if isinstance(citation, dict) else None
    for text, ctx in (
        (title, "pack.citation.title"),
        (body.get("description"), "pack.description"),
    ):
        if isinstance(text, str) and text:
            try:
                assert_clean(text, scope=CheckScope.USER_SHARED, context=ctx)
            except ValueError as exc:
                raise ForbiddenVocabInPack(str(exc)) from exc
...
DEFAULT_PACK: Final[LoadedPack] = load_builtin_pack("1.1.0")
"""빌트인 pack — module import 시 1 회 로드. snapshot_versions 의 active pack.

M7 #4 — v1.0.0 → v1.1.0 bump (price-return:total-annual 추가, ADR-0035 D4).
**§2.10 load-bearing**: v1.0.0.json 은 byte-불변(content_hash sha256:78e2d93f...
영구). 기존 frozen run 은 자기 snapshot.data_versions 의 factor_pack_version=="1.0.0"
+ frozen content_hash 를 그대로 보유하므로 reproduce 시 v1.0.0 을 로드 → 불변.
DEFAULT_PACK bump 는 신규 run 의 active pack content_hash 만 v1.1.0 으로 바꾼다."""
