        ...
        *,
        ...
        """Args:
            ...
            dividend_repo: cash_dividend repository (M7 #4, §2.10 break 수정). None
                ...
                정식 N/A — universe-상대 op (zscore/percentile/min_max_scale) 이 이
                field 를 쓰면 분포 모집단이 silent N/A 로 붕괴하던 갭 차단. 주입 시
                내부 DbFieldProvider 가 TotalReturnAdjuster 와 함께 배선돼 분포 모집단
                의 dividend / total return field 값을 종목별로 산출.
        """
        self.as_of = as_of
        ...
        # M7 #4 — dividend_repo 주입 시에만 total_return_adjuster 배선 (both-or-
        # neither — DbFieldProvider.__init__ assert 와 일관). stateless 라 1 인스턴스
        # 공유 (request 단위 provider 의 모든 종목 분포 평가가 같은 adjuster 재사용).
        self._total_return_adjuster = (
        ...
        **PIT** — 각 DbFieldProvider 는 as_of 고정 → effective_date <= as_of
        ...
        for code in self._list_active_codes(st):
            provider = DbFieldProvider(
                ...
                # M7 #4 (§2.10 break) — dividend / total return field 배선. 분포
                # 모집단 각 종목값이 silent N/A 가 아닌 같은 값으로 산출돼야
                # universe-상대 op (percentile/zscore/min_max) 이 정상 모집단.
                dividend_repo=self._dividend_repo,
                total_return_adjuster=self._total_return_adjuster,
