...
def _const_to_decimal(value: Any, *, canonical_id: str = "") -> Decimal:
    """상수(const/weights/lower/upper) → Decimal.

    **pack 포맷(ADR-0032 D1)은 decimal 문자열** — schema 가 const/weights/lower/upper 를
    `decimalString`(지수표기·float 금지) 로 강제하므로, 검증된 pack 에서 오는 값은 항상
    str 이다. 문자열은 `Decimal(str)` 로 정확 변환되어 content_hash cross-runtime 재현이
    자명하다(JCS 가 float 직렬화 단계를 거치지 않음).

    int/float 도 수용(robustness) — evaluator 단위 테스트가 AST 를 직접 구성해 number 를
    넘기거나, schema 우회 내부 경로에서. float 는 `str()` 경유로 IEEE 754 잔차
    (`Decimal(0.5)` = `0.5000000000000000111...`) 를 회피(oracle 결정 2). **단 float 는
    pack 포맷이 아님** — hash 대상 pack body 에는 schema 가 string 만 허용.

    inf / nan 은 거부 (oracle M1) — `_jcs.canonicalize_jcs` 의 `allow_nan=False` 와 일관.
    """
    ...
    if isinstance(value, str):
        # pack 포맷(ADR-0032 D1) — decimal 문자열. Decimal 이 직접 정확 파싱.
        # 비-finite/형식오류는 InvalidOperation → MalformedFormulaError.
        try:
            dec = Decimal(value)
        except (InvalidOperation, ValueError) as exc:
            raise MalformedFormulaError(
                f"{ctx}invalid const decimal string {value!r}: {exc}"
            ) from exc
    ...
    raise MalformedFormulaError(
        f"{ctx}const must be number or decimal string, "
