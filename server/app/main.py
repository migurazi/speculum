...
def create_app(
    *,
    ...
    # ADR-0031 D6 — 공시 사실추출기(LlmFactExtractor). 운영 LLM 연동은 release
    # blocker(§2.7 경계 + ADR-0006 자문)이므로 기본 미주입(None) → route 503(정상
    # 미제공 상태). 테스트/개발은 FakeLlmFactExtractor 주입으로 파이프라인 완전 동작.
    app.state.fact_extractor_override = fact_extractor
