...
/**
 * 상수 leaf — { const: string }.
 *
 * ADR-0032 D1 — 상수는 JSON number 가 아닌 **decimal 문자열**. content_hash 의
 * cross-runtime 재현(JCS)을 위해 float 직렬화(지수표기·IEEE 754 차이)를 봉쇄한다.
 * server schema 의 `decimalString` 과 1:1. 예: "0", "-1.5", "0.001".
 */
...
export interface ExprOp {
  ...
  readonly field?: string;
  readonly n?: number;
  // ADR-0032 D1 — 가중치/경계도 decimal 문자열(const 와 동일 이유). n 은 정수라 number.
...
export function isExprConst(e: Expr): e is ExprConst {
  // ADR-0032 D1 — const 는 decimal 문자열(과거 number 에서 변경).
  return typeof (e as ExprConst).const === "string"
...
export function renderFormula(e: Expr | null | undefined, relativeLabel?: string): string {
  ...
  if (isExprConst(e)) {
    // ADR-0032 D1 — const 는 이미 decimal 문자열. 그대로 표기(Fidelity).
    return e.const;
  }
  if (isExprField(e)) {
    return e.field;
  ...
  if (e.op === "weighted_sum") {
    ...
    const terms = args.map((a, i) => {
      ...
      // w 는 decimal 문자열(ADR-0032 D1) — 그대로 표기.
      return w === undefined ? rendered : `${rendered} × ${w}`;
    });
  ...
  if (e.op === "winsorize") {
    ...
    // lower/upper 는 decimal 문자열(ADR-0032 D1) — 그대로 표기.
    const lower = e.lower ?? "?";
    const upper = e.upper ?? "?";
...
export function makeOpSkeleton(op: AllowedOp): ExprOp {
  ...
  const node: {
    ...
    field?: string;
    n?: number;
    // ADR-0032 D1 — 상수류는 decimal 문자열. n 만 정수.
    weights?: string[];
    lower?: string;
    upper?: string;
  } = { op };
