...
export interface ReproduceResult {
  ...
  /**
   * pack 변조(content_hash 불일치) 여부 — M5 #3b / ADR-0033.
   * true = pack 변조 또는 hash 불일치로 재현 불가.
   * false = 정상 재현 또는 단순 pack 부재(slug/version 미존재).
   * client #6 use-시점 표시의 전제 — "변조" 와 "재로드 불가" 를 명확히 구분.
   */
...
export async function reproduceRun(
  ...
  return {
    ...
    note: wire.note,
    // pack_tampered → packTampered (M5 #3b — 변조 구조화 필드).
    packTampered: wire.pack_tampered,
  };
}
