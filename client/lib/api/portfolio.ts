...
interface PortfolioTransactionWire {
  ...
  // eslint-disable-next-line speculum/no-forbidden-words -- 거래 방향 wire 값("buy"/"sell" 은 server PortfolioTransaction.side 와 1:1 인 사실 분류 — 매매 권유 아님). 타입 리터럴이라 사용자에게 렌더되지 않으며, server 측은 동일 사실값을 forbidden-words conformance 에서 제외(tools/check_forbidden_words.py M3 hardening).
...
export async function listTransactions(
  ...
  // Array.isArray 의 타입가드는 `any[]` 기준이라 ReadonlyArray<T> 를 negative
  // 분기에서 narrow 하지 못한다(union 에 readonly array 가 남아 raw.transactions
  // 가 타입 에러). `"transactions" in raw` 로 객체 형태를 직접 narrow — true 면
  // ListWire(transactions 보유), false 면 배열.
  const wireList: ReadonlyArray<PortfolioTransactionWire> =
    "transactions" in raw ? raw.transactions : raw;
...
export async function listPositions(
  ...
  // Array.isArray 가 ReadonlyArray<T> 를 narrow 못 하는 문제 — transactions 와
  // 동일하게 `"positions" in raw` 로 객체 형태를 직접 narrow.
  const wireList: ReadonlyArray<PortfolioPositionWire> =
    "positions" in raw ? raw.positions : raw;
