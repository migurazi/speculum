    ...
    >
      <div className="flex flex-wrap items-center gap-2">
        ...
        {kind === "const" && isExprConst(node) ? (
          <input
            type="number"
            // ADR-0032 D1 — const 는 decimal 문자열. number 입력의 e.target.value
            // 는 이미 문자열이므로 그대로 emit(server validate 가 형식 권위).
            value={node.const}
...
function VariadicChildren({ node, onChange, depth }: OpChildrenProps): JSX.Element {
  ...
  const updateWeight = (index: number, value: string): void => {
    // ADR-0032 D1 — weights 는 decimal 문자열. 미설정 항은 "0".
        ...
        <div className="flex flex-wrap items-center gap-3 rounded-md bg-neutral-50 px-2 py-1">
          <label className="text-xs text-neutral-600">
            ...
            <input
              ...
              step="any"
              value={node.lower ?? "0"}
              onChange={(e) => onChange({ ...node, lower: e.target.value })}
          ...
          <label className="text-xs text-neutral-600">
            ...
            <input
              ...
              step="any"
              value={node.upper ?? "0"}
              onChange={(e) => onChange({ ...node, upper: e.target.value })}
