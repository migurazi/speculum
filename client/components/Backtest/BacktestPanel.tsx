 ...
 *   4. "재현 검증" 버튼 → POST /api/backtest/reproduce → matches/packTampered 배지.
 *   5. GET /api/data-freshness → KRX/DART 신선도 배지 (상단).
 *
 ...
 *   - 재현/신선도 배지: grayscale neutral — matches/isStale 는 사실 값,
 *     판단색(red/green/amber) 0 (ADR-0028 D3 / ADR-0033 D3).
 *
 * 색은 명명 상수(BACKTEST_PANEL_*) 로 export — backtest-visual-gate.test.tsx
 * 가 실제 렌더 색과 1:1 대조해 등락색/컬러맵 회귀를 자동 차단한다.
 *
 * 관련:
 *   - lib/api/backtest.ts — runBacktest / reproduceBacktest / getDataFreshness.
 *   - components/__tests__/backtest-visual-gate.test.tsx — 시각 게이트.
 *   - components/Backtest/__tests__/reproduce-trust-gate.test.tsx — 재현·신선도 게이트.
 */
...
/**
 * 재현 일치 배지 톤 — neutral grayscale. 사실값(일치 여부), 판단색 0.
 * ADR-0033 D3: "재현 일치"/"재현 불일치" 는 사실 — 수익/손실 판단색 금지.
 * reproduce-trust-gate.test.tsx 가 이 상수를 검증한다.
 */
export const BACKTEST_PANEL_REPRODUCE_MATCH = "bg-neutral-100 border-neutral-300 text-neutral-700";

/**
 * 재현 불일치 배지 톤 — neutral grayscale. 판단색(amber/red) 금지.
 * ADR-0033 D3: 불일치 = 사실 공시, 해석/판단 유도 없음.
 */
export const BACKTEST_PANEL_REPRODUCE_MISMATCH = "bg-neutral-200 border-neutral-400 text-neutral-800";

/**
 * pack 변조 감지 배지 톤 — neutral grayscale. 판단색 0.
 * ADR-0033 D5: "pack 변조 감지" 는 사실(content_hash 불일치) — 판단 아님.
 */
export const BACKTEST_PANEL_PACK_TAMPERED = "bg-neutral-200 border-neutral-400 text-neutral-800";

/**
 * 데이터 신선도 갱신 지연 배지 톤 — neutral grayscale. 판단색 0.
 * ADR-0033 D6: isStale=true 는 경과 사실 — "위험" 어휘/amber/red 금지.
 */
export const BACKTEST_PANEL_FRESHNESS_STALE = "bg-neutral-200 border-neutral-400 text-neutral-700";

/**
 * 데이터 신선도 정상 배지 톤 — neutral grayscale.
 */
export const BACKTEST_PANEL_FRESHNESS_OK = "bg-neutral-100 border-neutral-200 text-neutral-600";
...
/**
 * DataFreshnessPanel — KRX/DART 데이터 기준일 + 경과 일수 표시 (ADR-0033 D6).
 *
 * grayscale neutral 배지만 — 판단색(red/green/amber) 0.
 * isStale=true → "갱신 지연" neutral 배지(사실 공시, 판단 아님).
 * latestBatchAt=null → "데이터 없음" 사실 표시.
 */
function DataFreshnessPanel(): JSX.Element {
  const t = useTranslations("backtest");

  const { data, isError } = useQuery<DataFreshness>({
  ...
  if (isError) {
    return (
      <div
        ...
        className="rounded-md border border-neutral-200 bg-neutral-50 px-3 py-2 text-xs text-neutral-600"
      >
        {t("freshness.loadError")}
      </div>
    );
  }

  if (!data) {
    // 로딩 중 — 플레이스홀더 렌더(테스트에서 data-testid 탐색 가능).
    return (
      <div
        data-testid="data-freshness-panel"
  ...
  function renderSource(key: "krx" | "dart" | "kosis", freshness: DataFreshness["krx"]): JSX.Element {
    const label = t(`freshness.${key}`);
    const staleClass = cn(
    ...
    return (
      <div key={key} className="flex flex-wrap items-center gap-1.5">
        ...
        {freshness.latestBatchAt !== null ? (
          <>
            <span className="text-neutral-600">
              {t("freshness.batchAt", { date: freshness.latestBatchAt })}
            </span>
            {freshness.elapsedDays !== null ? (
              <span className="text-neutral-500">
                {t("freshness.elapsed", { days: freshness.elapsedDays })}
              </span>
            ) : null}
          </>
        ) : (
          <span className="text-neutral-500">{t("freshness.noData")}</span>
        )}
        {freshness.isStale ? (
          <span
            data-testid={`freshness-stale-${key}`}
  ...
  return (
    <div
      data-testid="data-freshness-panel"
      className="rounded-md border border-neutral-200 bg-neutral-50 px-3 py-2 text-xs space-y-1"
    >
      <p className="font-medium text-neutral-700 mb-1.5">{t("freshness.heading")}</p>
      {renderSource("krx", data.krx)}
      {renderSource("dart", data.dart)}
      {renderSource("kosis", data.kosis)}
      <p className="text-[11px] text-neutral-400 mt-0.5">
        {t("freshness.asOf", { asOf: data.asOf })}
      </p>
    </div>
  );
}
...
/**
 * ReproducePanel — "재현 검증" 버튼 + 결과 배지 (ADR-0033 D2~D5).
 *
 * 버튼 클릭 → POST /api/backtest/reproduce → matches/packTampered 배지.
 * 모든 배지: grayscale neutral — 판단색(red/green/amber) 0.
 * 문구: "재현 일치"(통과 사실) / "재현 불일치" / "pack 변조 감지" — 판단 어휘 금지.
 */
function ReproducePanel({ result }: ReproducePanelProps): JSX.Element {
  const t = useTranslations("backtest");

  const mutation = useMutation<BacktestReproduceResult, Error, void>({
    mutationFn: () =>
      reproduceBacktest(result.freeze, result.equityCurve),
  });
  ...
  return (
    <div
      data-testid="backtest-reproduce-panel"
      className="rounded-md border border-neutral-200 bg-neutral-50 px-3 py-2 space-y-2"
    >
      {/* 재현 검증 버튼 */}
      <button
        type="button"
        ...
        data-testid="backtest-reproduce-btn"
        className="inline-flex items-center rounded border border-neutral-300 bg-white px-3 py-1 text-xs font-medium text-neutral-700 hover:bg-neutral-50 disabled:cursor-not-allowed disabled:opacity-50"
      >
        {mutation.isPending ? t("reproduce.running") : t("reproduce.button")}
      </button>

      {/* 에러 */}
      {mutation.isError ? (
        <p
          data-testid="backtest-reproduce-error"
          className="text-xs text-neutral-700"
        >
          {t("reproduce.error", { message: mutation.error.message })}
        </p>
      ) : null}
      ...
      {mutation.data !== undefined ? (
        <div
          data-testid="backtest-reproduce-result"
          className="flex flex-wrap gap-2 items-center"
        >
          {/* matches 배지 — 사실(일치/불일치), 판단색 0 */}
          {mutation.data.matches ? (
            <span
              data-testid="backtest-reproduce-matches"
              className={cn(
            ...
            <span
              data-testid="backtest-reproduce-mismatch"
              className={cn(
                "inline-block rounded border px-2 py-0.5 text-xs font-medium",
                BACKTEST_PANEL_REPRODUCE_MISMATCH,
              )}
            >
              {t("reproduce.mismatch")}
            </span>
          )}

          {/* packTampered 배지 — 사실, 판단색 0 */}
          {mutation.data.packTampered ? (
            <span
              data-testid="backtest-pack-tampered"
              className={cn(
                "inline-block rounded border px-2 py-0.5 text-xs font-medium",
                BACKTEST_PANEL_PACK_TAMPERED,
              )}
            >
              {t("reproduce.packTampered")}
            </span>
          ) : null}

          {/* reproduceNote — 불일치 원인 사실 메모 */}
          {mutation.data.reproduceNote !== null &&
           mutation.data.reproduceNote.length > 0 ? (
            <p className="w-full text-[11px] text-neutral-500">
              {t("reproduce.note", { note: mutation.data.reproduceNote })}
            </p>
          ) : null}
        </div>
      ) : null}
...
function BacktestResultPanel({ result }: BacktestResultPanelProps): JSX.Element {
  ...
  return (
    ...
    >
      ...
      {/* ── 재현 검증 (ADR-0033 D2~D5) ──────────────────────────────────── */}
      <ReproducePanel result={result} />
