...
const SAVED_ITEM_1: SavedPackOut = {
  ...
  // ADR-0032 D3 — URL import 출처가 있는 pack(provenance 링크 렌더 검증).
  sourceUrl: "https://example.com/packs/my-pack.json",
};
...
function renderLibrary(
  ...
  renderWithIntl(
    <QueryClientProvider client={qc}>
      <PackLibrary pack={MOCK_PACK} sourceUrl={sourceUrl} onLoad={onLoad} />
    </QueryClientProvider>,
  );
  return qc;
}
...
describe("PackLibrary 저장 목록 렌더", () => {
  ...
  it("ADR-0032 D3 — sourceUrl 있으면 provenance 링크를 표시(없으면 미표시)", async () => {
    vi.mocked(listMyPacks).mockResolvedValue({
      items: [SAVED_ITEM_1, SAVED_ITEM_2],
    });
    renderLibrary();

    await screen.findByTestId("pack-library-table");
    // SAVED_ITEM_1 — provenance 행 + 외부 링크(보안 rel).
    const prov = screen.getByTestId(`pack-provenance-${SAVED_ITEM_1.id}`);
    const link = prov.querySelector("a");
    expect(link).not.toBeNull();
    expect(link?.getAttribute("href")).toBe(SAVED_ITEM_1.sourceUrl);
    expect(link?.getAttribute("rel")).toBe("noopener noreferrer");
    expect(link?.getAttribute("target")).toBe("_blank");
    // SAVED_ITEM_2 — 출처 없음 → provenance 행 미표시.
    expect(
      screen.queryByTestId(`pack-provenance-${SAVED_ITEM_2.id}`),
    ).toBeNull();
  });

  it("ADR-0032 D3 — 위험 scheme(http/javascript) sourceUrl 은 링크 미렌더", async () => {
    vi.mocked(listMyPacks).mockResolvedValue({
      items: [{ ...SAVED_ITEM_1, sourceUrl: "javascript:alert(1)" }],
    });
    renderLibrary();

    await screen.findByTestId("pack-library-table");
    // https 아님 → provenance 행 자체 미표시(클릭 XSS 차단).
    expect(
      screen.queryByTestId(`pack-provenance-${SAVED_ITEM_1.id}`),
    ).toBeNull();
  });
});
...
// ──────────────────────────────────────────────────────────────────────────────
// ADR-0032 D3 — 저장 시 provenance(sourceUrl) 전달
// ──────────────────────────────────────────────────────────────────────────────

describe("PackLibrary 저장 provenance(ADR-0032 D3)", () => {
  it("sourceUrl 사이드카가 있으면 savePack 에 provenance 를 함께 전달한다", async () => {
    vi.mocked(listMyPacks).mockResolvedValue({ items: [] });
    vi.mocked(savePack).mockResolvedValue(SAVED_ITEM_1);

    const url = "https://example.com/imported.json";
    renderLibrary(vi.fn(), undefined, url);
    await screen.findByTestId("pack-library-empty");

    await userEvent.click(screen.getByRole("button", { name: "현재 Pack 저장" }));

    await waitFor(() => {
      // pack + sourceUrl 동반 호출(row 메타 — content_hash 무관).
      expect(savePack).toHaveBeenCalledWith(MOCK_PACK, url);
    });
  });

  it("sourceUrl 이 없으면(직접 작성) savePack 에 null 을 전달한다", async () => {
    vi.mocked(listMyPacks).mockResolvedValue({ items: [] });
    vi.mocked(savePack).mockResolvedValue(SAVED_ITEM_2);
    ...
    await waitFor(() => {
      expect(savePack).toHaveBeenCalledWith(MOCK_PACK, null);
    });
  });
});
