...
/** null 통계값을 표시용 문자열로. */
function fmtStat(val: string | null): string {
  return val !== null ? val : "—";
}
  ...
  return (
    <tr className="border-t border-neutral-100">
      <td className="py-2.5 pr-4 text-sm text-neutral-800">
        ...
        {factor.unit ? (
          <span className="ml-1 text-xs text-neutral-400">{factor.unit}</span>
        ) : null}
      </td>
      ...
      <td className="py-2.5 pr-4 text-sm tabular-nums text-neutral-700">
        {hasData ? fmtStat(factor.mean) : "—"}
      </td>
      <td className="py-2.5 pr-4 text-sm tabular-nums text-neutral-700">
        {hasData ? fmtStat(factor.median) : "—"}
      </td>
      <td className="py-2.5 pr-4 text-sm tabular-nums text-neutral-700">
        {hasData ? fmtStat(factor.p25) : "—"}
      </td>
      <td className="py-2.5 pr-4 text-sm tabular-nums text-neutral-700">
        {hasData ? fmtStat(factor.p75) : "—"}
      </td>
      <td className="py-2.5 pr-4 text-sm tabular-nums text-neutral-700">
        {hasData ? fmtStat(factor.min) : "—"}
      </td>
      <td className="py-2.5 text-sm tabular-nums text-neutral-700">
        {hasData ? fmtStat(factor.max) : "—"}
      </td>
    </tr>
  );
}
